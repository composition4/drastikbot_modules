...
'''
Bot administration module for drastikbot.
...
class TheGrid():
    ...
    def register(self, password):  # register function used to make the owner account
        self.dbc.execute(
            '''CREATE TABLE IF NOT EXISTS TheGrid (user TEXT PRIMARY KEY, password TEXT, access TEXT, time INTEGER);''')
        if self.user == self.owner:
            self.dbc.execute('''INSERT INTO TheGrid (user, password, access, time) VALUES (?, ?, 'ALL', 0);''', (
                self.user, self.password_hash(password)))
    ...
    def login(self, password):
        ...
        if self.password_verify(password, fetch[0]):
            self.dbc.execute(
                '''UPDATE TheGrid SET time = strftime('%s', 'now') WHERE user=?;''', (self.user,))
            self.db.commit()
    ...
    def logout(self):
        if self.user in self.logged:
            ...
            self.dbc.execute(
                '''UPDATE TheGrid SET time = 0 WHERE user=?;''', (self.user,))
            self.db.commit()
    ...
    def useradd(self, user, password, channels):
        if self.user == self.owner:
            if ',' in channels:  # fragile maybe fix
                ...
                c = channels.split()
            c = json.dumps(c)
            self.dbc.execute('''INSERT INTO TheGrid (user, password, access, time) VALUES (?, ?, ?, 0);''', (
                user, self.password_hash(password), c))
            self.db.commit()
    ...
    def userpass(self, user, password):
        if self.user == self.owner or user == self.user:
            self.dbc.execute(
                '''UPDATE TheGrid SET password=? WHERE user=?;''', (self.password_hash(password), user))
            self.db.commit()
    ...
    def userlist(self):
        if self.user == self.owner:
            ...
            for f in self.dbc.fetchall():
                self.irc.notice(self.user, str(f).replace(
                    "'", '').strip("()").replace('"', ''))
    ...
    def module_setlist_add(self, mode, module, channel, botsys):
        ...
        if (self.user == self.owner) or (channel in self.admins[self.user]):
            try:
                self.config_r['irc']['modules'][
                    'settings'][module][mode].append(channel)
            except KeyError:
                self.config_r['irc']['modules'][
                    'settings'][module] = {mode: [channel]}
            except KeyError:
                self.config_r['irc']['modules'][
                    'settings'] = {module: {mode: [channel]}}
            except KeyError:
    ...
    def module_setlist_list(self, module):
        try:
            ...
            return self.irc.notice(self.user, 'No blacklist or whitelist set for module {}.'.format(module))
    ...
    def set_chan_prefix(self, channel, prefix):
        if (self.user == self.owner) or (channel in self.admins[self.user]):
            try:
                self.config_r['irc']['channels']['settings'][
                    channel]['prefix'].append(prefix)
            except KeyError:
                self.config_r['irc']['channels'][
