...
def short_url(url):
    service = 'http://tinyurl.com/create.php?source=indexpage&url={}&submit=Make+TinyURL%21&alias='.format(url)
...
def ddg(query, query_p):
    if query_p[0] == '!':
        search = 'http://api.duckduckgo.com/?q={}&format=json&no_redirect=1'.format(query)
        r = requests.get(search, timeout=2)
        ...
        r = requests.get(search, timeout=2)
        soup = bs4.BeautifulSoup(r.text, "html.parser")
        result = soup.find('a', {'class': ['result__url']}, href=True).get('href')
...
def main(cmd, info, db, irc):
    ...
    if cmd == 'g':
        irc.privmsg(info[0], '\x0302G\x0304o\x0308o\x0302g\x0309l\x0304e\x0F: {}'.format(google(query)))
    elif cmd == 'yt':
        ...
        irc.privmsg(info[0], '\x0315DuckDuckGo\x0F: {}'.format(ddg(query, query_p)))
    elif cmd == 'searx':
        irc.privmsg(info[0], '\x0315sear\x0311X\x0F: {}'.format(searx(query)))
    elif cmd == 'sp':
        irc.privmsg(info[0], '\x0304start\x0302page\x0F: {}'.format(startpage(query)))
